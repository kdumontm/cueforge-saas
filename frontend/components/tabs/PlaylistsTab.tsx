  ...
  return (
    <div className="space-y-4 p-4">
      ...
      <div className="space-y-2">
        {playlists.length === 0 ? (
          <p className="text-sm text-[var(--text-muted)] p-3">Pas encore de playlists</p>
        ) : (
          playlists.map((playlist) => (
            <button
              key={playlist.id}
              onClick={() => onSelect?.(playlist)}
              className="w-full text-left flex items-center gap-3 p-3 rounded-lg bg-[var(--bg-elevated)] border border-[var(--border-subtle)] hover:border-[var(--border-default)] transition-colors"
            >

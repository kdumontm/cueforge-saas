        ...
        <div className="p-4 min-h-[160px]">
          {activeTab === 'cues' && (
            <CuesTab
              ...
              cuePoints={selectedRawTrack?.cue_points || []}
              onCreateCue={(cue) => console.log('Create cue:', cue)}
              onDeleteCue={(id) => console.log('Delete cue:', id)}
            />
          )}
          {activeTab === 'beatgrid' && (
            <BeatgridTab
              track={selectedRawTrack}
              beatgrid={selectedRawTrack?.analysis ? { bpm: selectedRawTrack.analysis.bpm, downbeat_ms: 0, locked: false } : undefined}
            />
          )}
          {activeTab === 'stems' && <StemsTab track={selectedRawTrack} />}
          {activeTab === 'eq' && <EQTab />}
          {activeTab === 'fx' && <FXTab />}

...
import {
  Shield, ChevronRight, LogOut, Sun, Moon, Menu, X, Bell,
  Search, PanelLeftClose, PanelLeft,
} from "lucide-react";
import { ToastProvider } from "./_components/shared";
import { AdminModulesProvider, useAdminModules } from "./_components/AdminModulesContext";
    ...
    <ToastProvider>
      <div className="flex h-screen bg-bg-primary overflow-hidden">
        ...
        >
          ...
          {/* Nav groups — filtered by enabled modules */}
          <nav className="flex-1 py-2 px-2 overflow-y-auto space-y-4">
            {NAV_GROUPS.map((group) => {
              // Filter items: keep essential + enabled modules
              const visibleItems = group.items.filter(
                (item) => ESSENTIAL_MODULE_IDS.has(item.id) || isEnabled(item.id)
              );

...
from app.models import community_cues  # noqa: F401 — registers CommunityCue with Base
...
async def lifespan(app: FastAPI):
    ...
    # 15.5. F : Watchdog — surveille les tracks bloqués en pending/analyzing
    try:
        ...
        logger.info("✅ Analysis watchdog started (daemon thread)")
    except Exception as e:
...
_safe_mount("app.routers.improvements", "/api/v1", ["improvements"])  # AVANT tracks pour eviter /tracks/{track_id} collision
...
# compare déplacé avant tracks plus haut (sinon /{track_id} intercepte)

"""
Organization management router — create org, invite members, manage roles.
Mount: app.include_router(org_management.router, prefix="/api/v1/org", tags=["organization-management"])
"""
...
def _plan_max_members(plan: str) -> int:
    return {"free": 1, "pro": 5, "enterprise": 50, "unlimited": 100}.get(plan, 1)
...
async def create_org(data: OrgCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if user.organization_id:
        raise HTTPException(status_code=400, detail="Already in an organization")
    org = Organization(
        name=data.name, slug=_generate_slug(data.name), owner_id=user.id,
...
async def get_org(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if not user.organization_id:
...
async def update_org(data: OrgUpdate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def list_members(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if not user.organization_id:
        raise HTTPException(status_code=403, detail="No organization")
...
async def update_member_role(user_id: int, data: RoleUpdate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def remove_member(user_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def invite_member(data: InviteCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    org = _require_org_admin(user)
    ...
    existing = db.query(OrgInvite).filter(
        OrgInvite.organization_id == org.id, OrgInvite.email == data.email, OrgInvite.status == "pending"
    ).first()
    if existing:
        raise HTTPException(status_code=400, detail="Invite already pending")
    invite = OrgInvite(
        organization_id=org.id, email=data.email, role=data.role,
        invited_by=user.id, expires_at=datetime.utcnow() + timedelta(days=7),
    )
    db.add(invite)
    db.commit()
    db.refresh(invite)
...
async def list_invites(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    _require_org_admin(user)
    invites = db.query(OrgInvite).filter(
        OrgInvite.organization_id == user.organization_id, OrgInvite.status == "pending"
    ).all()
    return [InviteResponse.model_validate(i) for i in invites]

@router.post("/invite/{token}/accept")
async def accept_invite(token: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    invite = db.query(OrgInvite).filter(OrgInvite.token == token, OrgInvite.status == "pending").first()
    if not invite:

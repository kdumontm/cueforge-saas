...
def _should_create_job_failure_feedback(track_id: int, job_type: str, db: Session) -> bool:
    """
    Vérifie si on doit créer un feedback pour ce job failure.
    Rate limit: max 1 feedback par track+job_type par heure (dédup).
    """
    # Cherche si un feedback admin existe déjà pour cette track dans la dernière heure

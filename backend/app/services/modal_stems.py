...
def separate_stems_modal(
    ...
    logger.info(f"[MODAL] Requesting GPU stem separation for track {track_id}")
    start = time.time()
...
def separate_stems_with_fallback(
    ...
    if is_modal_available():
        stems = separate_stems_modal(track_id, audio_url)
        if stems:
            return stems
        logger.info(f"[MODAL] Fallback to CPU for track {track_id}")

    # ── Fallback Demucs CPU local (~20-40s) ──
    from app.services.stem_analysis import separate_stems as cpu_separate
    return cpu_separate(file_path)
